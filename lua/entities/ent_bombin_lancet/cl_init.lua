...
-- Lancet is a small drone: tight offsets (±25 lateral, ±30 body)
-- ================================================================
...
-- Lancet is small (~40 units long) — use tight scatter
-- ----------------------------------------------------------------
local TIER_OFFSETS = {
	[1] = {
		{ x =  15, y =  10, z = 3 },
...
local function SpawnBurstFX(ent, tier)
	...
	for i = 1, count do
		local offset = fwd * math.Rand(-30, 30)
...
-- Net receiver
-- ----------------------------------------------------------------
net.Receive("bombin_lancet_damage_tier", function()
	...
	if not IsValid(ent) then
		LancetStates[idx] = LancetStates[idx] or { tier = 0, particles = {}, nextBurst = 0, pendingTier = nil }
		LancetStates[idx].pendingTier = tier
		return
	end

	local state = LancetStates[idx] or { tier = 0, particles = {}, nextBurst = 0, pendingTier = nil }
	LancetStates[idx] = state
	state.tier = tier
...
-- ----------------------------------------------------------------
hook.Add("Think", "bombin_lancet_damage_fx", function()
	local ct = CurTime()
	for idx, state in pairs(LancetStates) do
		local ent = ents.GetByIndex(idx)
...
function ENT:OnRemove()
	local state = LancetStates[self:EntIndex()]
	if state then
		for _, p in ipairs(state.particles) do
			if IsValid(p) then p:StopEmission() end
		end
		LancetStates[self:EntIndex()] = nil
	end
end
